...
```{r Modello lineare punti vs tipo missioni}
missioni_players1$subType = recode(missioni_players1$subType, 'cib' = 1, 'double' = 2, 'ticket-punti' = 3)
lm_missioni = lm(points ~ subType, subset(missioni_players1, !(is.na(subType))))
summary(lm_missioni)
```

So we plot the regression line on the grouped boxplots:
```{r Plot lineare punti vs tipo missioni, fig.align='center', warning=FALSE, message=FALSE}

graph<-ggplot(missioni_players1, aes(x=factor(subType), y=points, group = factor(subType)))+
  geom_boxplot()+
  stat_summary()+
  stat_boxplot(geom ='errorbar', width = 0.5) +
  labs(title = 'Linear regression Punti vs Type of mission', x = 'Type of mission', y = 'Punti') +
  scale_x_discrete(labels = c('cib', 'double', 'ticket'))
graph <-graph + geom_abline(intercept = lm_missioni$coefficients[1], slope = lm_missioni$coefficients[2], color = 'blue')
graph
```
...
```{r Modello lineare punti vs categoria prodotti}
prodotti1 <- prodotti

prodotti1$TIER = recode(prodotti1$TIER, 'NO TIER' = 1, 'TIER3' = 2, 'TIER2' = 3, 'TIER1' = 4)
lm_prodotti = lm(points ~ TIER, subset(prodotti1, !(is.na(TIER))))
summary(lm_prodotti)
```

So we plot the regression line on the grouped boxplots:
```{r Plot lineare Punti vs Categoria, fig.align='center'}

graph<-ggplot(subset(prodotti1, !(is.na(TIER))), aes(x=factor(TIER), y=points, group = factor(TIER)))+
  geom_boxplot()+
  stat_summary()+
  stat_boxplot(geom ='errorbar', width = 0.5)+
  labs(title = 'Linear regression Punti vs TIER', x = 'TIER', y = 'Punti') +
  scale_x_discrete(labels = c('NO TIER', 'TIER3', 'TIER2', 'TIER1'))
graph <-graph + geom_abline(intercept = lm_prodotti$coefficients[1], slope = lm_prodotti$coefficients[2], color = 'blue')
graph
```
We use a linear regression model to test the significance of the link between the dependent variable points and the independent variable TIER. The p-value has a very small value, so the relationship is significant (an ANOVA would be better).
...
We made a clusterization of the column *Regione* with three clusters: *Nord*, *Center*, *Sud.* Finally a summary of the final dataset:
```{r}
...
ggplot(data, aes(x = clust, y = ETA_MM_BambinoTODAY-6)) +
  ...
  stat_boxplot(geom ='errorbar', width = 0.3) +
  stat_summary(fun.y=mean, geom="point") + 
  labs(title = "Boxplots childs' age when survey started", x = 'Cluster', y = "Età (mesi)")
```
...
*recency > avg_between + 2 ∗ std_between*  
...
M<-cor(frequencies_general_data)
...
For each repetition, the actual and predicted classifications have been matched so as to fill the 2 × 2 confusion matrices and assess the large-sample predictive performance via Balanced Accuracy, precision, recall and the F-measure.
...
Confusion matrix
```{r, fig.align = "center"}
cm = confusionMatrix(y_test_outer,predictions_dt$.pred_class)
draw_confusion_matrix(cm)
```
...
```{r, fig.align= "center"}
cm = confusionMatrix(y_test_outer,predictions_rf$.pred_class)
draw_confusion_matrix(cm)
```
...
We notice that the logistic regression doesn't have an accuracy close to the ones of the decision tree and random forest. But it can give us other information about feature reduction. While selecting the best fit we printed the best parameters' combination and the coefficients of the net regression.
```{r}
coeff = lr_fit$fit$beta
coefficients = lr_fit$fit$beta[1:7, dim(coeff)[2]]
coefficients 
```
We can plot the trend of the coefficients respect to lambda's values.
```{r, fig.align = 'center'}
plotting_model_cv(lr_fit$fit, round(best_lg$mixture, 2))
```
The lasso regression uses the l1 norm, so it can make a coefficient equal to 0. At the extreme left-hand side of the plot the corresponding lasso coefficient estimates are the same as the usual least squares estimates. But as λ increases, the lasso coefficient estimates shrink towards zero. When λ is extremely large, then all of the coefficients' estimates are basically zero; this corresponds to the null model that contains no predictors. 


# Conclusion

Reducing churn is extremely important in competitive markets since acquiring new customers is very difficult, for this reason this report aimed to understand the mum’s behavior and build a system that prevent churning using a predictive model. This prediction model needs to achieve high Recall values to have a model that ideally does not miss any churns. We have applied feature engineering, effective feature transformation approach to make the features ready for machine learning algorithms. The algorithms employed are Decision Tree, Random Forest and Logistic Regression. We compared models by using a repeated stratified nested cross validation.The experimental results show that the tree based algorithms outperformed Logistic Regression in terms of all the performance measures such as balanced accuracy, precision, recall and AUC score. It is interesting to note that only behavioral data was used - no demographic data was included the model. By using more complex and game specific features, a better classification performance could possibly be achieved.
